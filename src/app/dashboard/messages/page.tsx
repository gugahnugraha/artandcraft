...
import { MessageSquare, Send, Store, User, Loader2, Image as ImageIcon } from "lucide-react";
...
export default function BuyerMessagesPage() {
  ...
  useEffect(() => {
    if (activeConvId) {
      ...
      // Setup polling every 3 seconds for messages
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col space-y-4">
      <div>
        <h1 className="font-serif text-2xl font-bold text-foreground flex items-center gap-2">
          <MessageSquare className="h-6 w-6 text-primary" /> Pesan & Diskusi
        </h1>
        <p className="text-xs text-muted-foreground mt-0.5">
          Tanya pengrajin mengenai ketersediaan, custom order, atau detail produk.
        </p>
      </div>
          ...
          {isLoadingConvs ? (
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-border/40">
              {conversations.map((conv) => {
                ...
                const isSeller = conv.sellerProfile.userId === session.user.id;
                const partnerName = isSeller ? conv.buyer.name : conv.sellerProfile.storeName;
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline mb-1">
                        <p className="font-bold text-sm text-foreground truncate">{partnerName}</p>
                      </div>
        ...
        <div className="hidden sm:flex flex-1 flex-col h-full bg-background">
          {activeConvId && activeConversation ? (
            <>
              ...
              <div className="p-4 border-b border-border flex items-center gap-3 bg-card shrink-0">
                <div className="h-9 w-9 rounded-full bg-primary/10 text-primary font-bold text-sm flex items-center justify-center">
                  {activeConversation.sellerProfile.storeName.charAt(0)}
                </div>
                <div>
                  <h3 className="font-bold text-sm text-foreground">
                    {activeConversation.sellerProfile.storeName}
                  </h3>
                  <p className="text-xs text-muted-foreground">Pengrajin Terverifikasi</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSendMessage} className="p-3 border-t border-border bg-card flex items-center gap-2">
                <input
                  ...
                  placeholder="Ketik pesan untuk pengrajin..."
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
              ...
              <p className="font-semibold text-sm">Pilih percakapan untuk memulai percakapan</p>
            </div>
          )}
        </div>
      </div>
