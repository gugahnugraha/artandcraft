  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "AWAITING_PAYMENT":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-500/10 text-orange-600"><Clock className="h-3.5 w-3.5" /> {t.seller_orders.status_awaiting}</span>;
      case "PAID":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-600"><CheckCircle2 className="h-3.5 w-3.5" /> {t.seller_orders.status_paid}</span>;
      case "PROCESSING":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-500/10 text-purple-600"><Package className="h-3.5 w-3.5" /> {t.seller_orders.status_processing}</span>;
      case "SHIPPED":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-green-500/10 text-green-600"><Truck className="h-3.5 w-3.5" /> {t.seller_orders.status_shipped}</span>;
      default:
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl font-bold text-foreground">{t.seller_orders.title}</h1>
          <p className="text-muted-foreground text-sm mt-1">
            {t.seller_orders.subtitle}
          </p>
        </div>
      </div>
      ...
      {groupedOrders.length === 0 ? (
        <div className="bg-card rounded-2xl border border-border p-12 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">{t.seller_orders.empty_title}</h2>
          <p className="text-muted-foreground text-sm max-w-md">
            {t.seller_orders.empty_desc}
          </p>
        </div>
      ) : (
        <div className="space-y-6">
              ...
              <div key={orderInfo.id} className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                ...
                <div className="bg-muted/30 px-6 py-4 border-b border-border/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t.seller_orders.ordered_at}{new Date(orderInfo.createdAt).toLocaleString(lang === "en" ? "en-US" : "id-ID")}
                    </p>
                  </div>
                  <div className="text-left sm:text-right">
                    <p className="text-xs text-muted-foreground mb-1">{t.seller_orders.total_revenue}</p>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <h3 className="text-sm font-semibold text-foreground border-b border-border/50 pb-2">{t.seller_orders.product_list}</h3>
                    {items.map((item: any) => (
                  ...
                  <div className="space-y-4 md:border-l md:border-border/50 md:pl-6">
                    <h3 className="text-sm font-semibold text-foreground border-b border-border/50 pb-2">{t.seller_orders.shipping_info}</h3>
                    {orderInfo.shippingAddress ? (
                      ...
                      <p className="text-sm text-muted-foreground flex items-center gap-2">
                        <AlertCircle className="h-4 w-4" /> {t.seller_orders.no_address}
                      </p>
                    )}
